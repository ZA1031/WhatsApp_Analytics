...
import {
    ...
    BlockResult,
    BlockResultMessage,
    ContainingMessagesMessage,
    CopiedMessagesMessage,
    InitMessage,
    MessagesAll,
    ReadyMessage,
...
export declare interface WorkerWrapper {
    ...
    emit(event: "copied-messages-updated", copiedMessages: any[]): boolean; // Add this line
    emit(event: "containing-messages-updated", copiedMessages: any[]): boolean; // Add this line
    emit(event: "messages-all", copiedMessages: any[]): boolean; // Add this line
    ...
    on(event: "copied-messages-updated", listener: (copiedMessages: any[]) => void): this; // Add this line
    on(event: "containing-messages-updated", listener: (copiedMessages: any[]) => void): this; // Add this line
    on(event: "messages-all", listener: (copiedMessages: any[]) => void): this; // Add this line
}
...
export class WorkerWrapper extends EventEmitter {
    ...
    private onMessage(e: MessageEvent<ReadyMessage | BlockResultMessage<BlockKey> | CopiedMessagesMessage | ContainingMessagesMessage | MessagesAll>) {
        const res = e.data;
        if (res.type === "ready") {
        ...
        } else if (res.type === "update-copied-messages") {
            this.emit("copied-messages-updated", res.copiedMessages);
        } else if (res.type === "update-containing-messages") {
            this.emit("containing-messages-updated", res.containingMessages);
        } else if (res.type === "messages-all") {            
            this.emit("messages-all", res.messagesAll);
        }
    }
