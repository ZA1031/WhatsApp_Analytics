...
export class MessageView implements Message {
    id: Index = -1; // Add id property
    text?: string; // Add text property (optional)
    ...
    constructor(
        ...
        private readonly bitConfig: MessageBitConfig,
        private readonly dateKeys: string[] // Pass dateKeys to the constructor
    ) {
        // Deserialize id
        this.id = stream.readVarInt(); // Assuming id is serialized as a variable-length integer
        ...
        this.date = dateKeys[this.dayIndex]; // Map dayIndex to date
        ...
        if ((this.flags & MessageFlags.Text) > 0) {
            ...
            if (textLength > 0) {
                ...
                this.text = new TextDecoder().decode(textBytes); // Convert bytes to string
            } else {
                this.text = ""; // No text
            }
        }
    ...
    get reply(): MessageView | undefined {
        if (this.hasReply) {
            ...
            return new MessageView(this.stream, this.bitConfig, this.dateKeys);
        }
        return undefined;
    }
